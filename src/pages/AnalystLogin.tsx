...
import { BarChart2, Lock, Eye, EyeOff, AlertCircle } from 'lucide-react';
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.role !== 'analyst' && data.role !== 'admin') {
          setError('Access denied. Analyst privileges required.');
          return;
        }
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 selection:bg-indigo-500/30">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Analyst Portal</h1>
          <p className="text-slate-500 mt-2 font-medium">Professional market research and investment analysis.</p>
        </div>
        ...
        <div className="bg-white rounded-3xl p-8 shadow-xl shadow-slate-200/50 border border-slate-100 relative overflow-hidden">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="analyst@sqplatform.com"
            ...
            >
              {loading ? 'Authenticating...' : 'Sign In to Analyst Portal'}
          ...
          <div className="mt-8 pt-6 border-t border-slate-100 flex flex-col gap-3 text-xs text-slate-500">
            <div className="flex items-center justify-between">
              <Link to="/analyst-signup" className="text-indigo-600 font-semibold hover:underline">Sign Up</Link>
              <button
                type="button"
                onClick={() => openRecoveryModal(email)}
